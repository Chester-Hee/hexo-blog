...
上传文件到远程时，我们都会遇到同样一个问题，**文件过大**该怎么处理？
当然，有两种方法解决：
...
如果只考虑第一种方案，前端压缩图片后上传，那么对于非图片类型的文件上传又要考虑非图片类型文件的压缩方法。第二种方式，实现分块上传对编码要求更高，后端和前端都需要实现可靠的组件。

本篇文章，我们**只讨论如果使用前端压缩图片**

我画了一张图，描述 JavaScript 压缩图片整个流程调用
